...
<html>
         ...
         <!-- Features -->
            <section id="content">
                <div class="container">
                  ...
                  <input type="VARCHAR" name = "pName" VALUE='모더나' aria-label="Small" size="20" placeholder="상품 이름를 검색해주세요">
   ...
   while($row4=mysqli_fetch_array($ret4))
   {
    ...
    if($row4['registrationDate']<date('Y-m-d') and $row4['pharmacistName']=='모더나'){
      $stock+=$row4['totalProduction'];
    }
   }
   while($row5=mysqli_fetch_array($ret5))
   {
      if ($row5['orderDate']<date('Y-m-d')and$row5['pharmacistName']=='모더나'){
   ...
   if($bool == FALSE)
   {
      $sql2 = "SELECT *FROM vaccine WHERE ('$currentdate'>registrationDate) AND (pharmacistName='모더나')";
      ...
      while($row=mysqli_fetch_array($ret2)){
         ...
         echo "<TD>", "<a href='update_moderna.php?uniqueNumber=", $row['uniqueNumber'], "'>수정</a></TD>";
         echo "<TD>", "<a href='delete_moderna.php?uniqueNumber=", $row['uniqueNumber'], "'>삭제</a></TD?"; 
         echo "</TR>";
      }
   
      mysqli_close($con);
      echo "</TABLE>";
      echo "<br> <a href='moderna.php'><--초기 화면</a>";
