<?php
  ...
  $pssn = "select * from patient WHERE ((dateofDeath != 0000-00-00) AND (dateofDeath != 0000-00-00)) AND(hospitalName='경북대병원')";
  $data = mysqli_query($con, $pssn);
  $pssn_rows = mysqli_num_rows($data);
  
  $epssn = "select * from patient WHERE ((dateofDeath != 0000-00-00) AND (dateofDeath != 0000-00-00)) AND (diseaseName='코로나') AND (hospitalName='경북대병원')";
  $edata = mysqli_query($con1, $epssn);
  $epssn_rows = mysqli_num_rows($edata);
  
  $npssn = "select * from patient WHERE ((dateofDeath != 0000-00-00) AND (dateofDeath != 0000-00-00)) AND (diseaseName!='코로나') AND (hospitalName='경북대병원')";
               ...
               <div class="container">
                  <div class="row">
                     <div class="col-12">
                        ...
                        <!-- Logo -->
                           <h1><a href="KNUhospital3_1.php" id="logo"><span style="color:#ffffff">KNU Medical Center</a></h1>
               ...
               <div class="container">
                  <div class="row">
                     ...
                     <h1> ▶보유 병상 수 조회 결과 </h1>
                         <table class='bluetop'>
                             <TR>
                             <TH> 경북대병원 </TH><TH>보유 병상</TH><TH>가용 병상</TH>
                            </TR>
                            <TR>
                            <TH>전체</TH> 
                     <?php
