...
<html>
   ...
   <body>
               ...
               <div class="container">
                  <div class="row">
                     <div class="col-12">
                        ...
                        <!-- Logo -->
                           <h1><a href="KNUhospital.php" id="logo"><span style="color:#ffffff">KNU Hospital</a></h1>
            ...
            <section id="content">
                <div class="container">
                    <div class="row">
                        ...
                        <?php echo "<a href='KNUhospital.php'> 이전 화면</a>"; ?>
                        <h1> 조회하기 </h1>
						...
						<input type="VARCHAR" name="diseaseName" aria-label="Small" size="20" placeholder="병명으 검색">
	...
	if($bool == FALSE)
	{
		$sql2 = "SELECT *FROM patient WHERE (hospitalName='경북대병원')";
...
<?php
	...
	if(isset($bool1)) /*id set*/
	{
		if($bool2) /*id,  pnameset*/
		{
			if($bool3) /*id,  pnameset, hospitalizationDate*/
			{
				if($bool4) /* 전체 set*/
				{
					$sql = "SELECT *FROM patient WHERE (pssn='".$_GET["id"]."' AND pname='".$_GET["pname"]."' AND hospitalizationDate='".$_GET["hospitalizationDate"]."' AND diseaseName='".$_GET["diseaseName"]."')";
				}
				else /*diseaseName뺴고 다*/
				{
					$sql = "SELECT *FROM patient WHERE (pssn='".$_GET["id"]."' AND pname='".$_GET["pname"]."' AND hospitalizationDate='".$_GET["hospitalizationDate"]."')";
				}
			}
			else if($bool4)
			{
				$sql = "SELECT *FROM patient WHERE (pssn='".$_GET["id"]."' AND pname='".$_GET["pname"]."' AND diseaseName='".$_GET["diseaseName"]."')";
			}
			else
			{
				$sql = "SELECT *FROM patient WHERE (pssn='".$_GET["id"]."' AND pname='".$_GET["pname"]."')";
			}
		}
		else /*id set, regist- unset*/
		{
			if($bool3)
			{
				if($bool4)
				{
					$sql = 'SELECT *FROM patient WHERE (pssn="'.$_GET["id"].'" AND hospitalizationDate="'.$_GET["hospitalizationDate"].'" AND diseaseName="'.$_GET["diseaseName"].'")';
				}
				else
				{
					$sql = 'SELECT *FROM patient WHERE (pssn="'.$_GET["id"].'" AND hospitalizationDate="'.$_GET["hospitalizationDate"].'")';
				}
			}
			else if($bool4)
			{
				$sql = 'SELECT *FROM patient WHERE (pssn="'.$_GET["id"].'" AND diseaseName="'.$_GET["diseaseName"].'")';
			}
			else
			{
				$sql = 'SELECT *FROM patient WHERE (pssn="'.$_GET["id"].'")';
			}
		}
	...
	else
	{
		if($bool2) /*re*/
		{
			if($bool3) /*va*/
			{
				if($bool4)  /*pNa*/
				{
					$sql = 'SELECT *FROM patient WHERE (pname="'.$_GET["pname"].'" AND hospitalizationDate="'.$_GET["hospitalizationDate"].'" AND diseaseName="'.$_GET["diseaseName"].'")';
				}
				else 
				{
		...
		else /*id set, regist- unset*/
		{
			if($bool3)
			{
				if($bool4)
				{
					$sql = 'SELECT *FROM patient WHERE (hospitalizationDate="'.$_GET["hospitalizationDate"].'" AND diseaseName="'.$_GET["diseaseName"].'")';
				}
				else
				{
			...
			{
				$sql = "SELECT *FROM patient WHERE (hospitalName='경북대병원')";
			}
		}
	}
   ...
   echo "<TABLE class='bluetop'>";
   echo "<TR>";
   echo "<TH>pname</TH><TH>pssn</TH><TH>age</TH><TH>tel</TH><TH>address</TH><TH>gender</TH><TH>hospitalizationDate</TH><TH>dischargeDate</TH><TH>dateOfDeath</TH><TH>covidInspectionDate</TH><TH>covidConfirmation</TH><TH>diseaseName</TH><TH>hospitalName</TH>";
   echo "<TH>수정</TH><TH>삭제</TH>";
   echo "</TR>";
   while($row=mysqli_fetch_array($ret)){
      echo "<TR>";
	    ...
	    echo "<TD>", $row['dateOfDeath'], "</TD>";
        echo "<TD>", $row['covidInspectionDate'], "</TD>";
	    echo "<TD>", $row['covidConfirmation'], "</TD>";
	    echo "<TD>", $row['diseaseName'], "</TD>";
	    echo "<TD>", $row['hospitalName'], "</TD>"; 
	 	echo "<TD>", "<a href='YNU_update_patient.php?pssn=", $row['pssn'], "'>수정</a></TD>";
		echo "<TD>", "<a href='YNU_delete_patient.php?pssn=", $row['pssn'], "'>삭제</a></TD?"; 
